...
import { PollDesc } from 'src/app/app.models';
...
export class PollItemComponent implements OnInit {
  @Input() pollDesc: PollDesc = new PollDesc();
  @Input() viewType = 'grid';
  ...
  ngOnChanges(changes: {[propKey: string]: SimpleChange}){
    if(changes.viewColChanged){
      ...
      if(!changes.viewColChanged.isFirstChange()){
        // if(this.project.gallery.length > 1){
 ...
 */
  public onCompare(){
    return this.appService.Data.compareList.filter(item=>item.id === this.pollDesc.poll.id)[0];
  }
  ...
  public onFavorites(){
    return this.appService.Data.favorites.filter(item=>item.id === this.pollDesc.poll.id)[0];
  }
